<template>
  <div class="workflow-view">
    ...
    <div class="content">
      <div v-if="activeTab === 'yaml'" class="yaml-editor sakura-panel">
        <div v-if="yamlParseError" class="yaml-error">YAML Parse Error: {{ yamlParseError }}</div>
        <textarea
      ...
      <div v-if="activeTab === 'graph'" class="vueflow-container sakura-panel" ref="vueflowContainerRef">
        <VueFlow
          v-model:nodes="nodes"
          v-model:edges="edges"
          ...
          <template #node-workflow-node="props">
            <WorkflowNode :id="props.id" :data="props.data" @hover="onNodeHover" @leave="onNodeLeave" />
          </template>
          ...
          >
            <template v-if="contextMenuType === 'pane'">
              <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.createNode" placement="right">
                <div class="context-menu-item" @click.stop="() => { hideContextMenu(); openCreateNodeModal(); }">Create Node</div>
              </RichTooltip>
              <div v-else class="context-menu-item" @click.stop="() => { hideContextMenu(); openCreateNodeModal(); }">Create Node</div>
            </template>
            ...
            <template v-else-if="contextMenuType === 'node'">
              <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.copyNode" placement="right">
                <div class="context-menu-item" @click.stop="() => { hideContextMenu(); onCopyNodeFromContext(); }">Copy Node</div>
              </RichTooltip>
              <div v-else class="context-menu-item" @click.stop="() => { hideContextMenu(); onCopyNodeFromContext(); }">Copy Node</div>
              <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.deleteNode" placement="right">
                <div class="context-menu-item" @click.stop="() => { hideContextMenu(); onDeleteNodeFromContext(); }">Delete Node</div>
              </RichTooltip>
              <div v-else class="context-menu-item" @click.stop="() => { hideContextMenu(); onDeleteNodeFromContext(); }">Delete Node</div>
            </template>
            ...
            <template v-else-if="contextMenuType === 'edge'">
              <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.deleteEdge" placement="right">
                <div class="context-menu-item" @click.stop="() => { hideContextMenu(); onDeleteEdgeFromContext(); }">Delete Edge</div>
              </RichTooltip>
              <div v-else class="context-menu-item" @click.stop="() => { hideContextMenu(); onDeleteEdgeFromContext(); }">Delete Edge</div>
            </template>
          </div>
        </transition>
      </div>
    ...
    <div class="tabs sakura-panel">
      <div class="tab-buttons">
        <button :class="['tab', { active: activeTab === 'graph' }]" @click="activeTab = 'graph'">Workflow Graph</button>
        <button :class="['tab', { active: activeTab === 'yaml' }]" @click="activeTab = 'yaml'">YAML Configuration</button>
      </div>
      <div v-if="activeTab === 'graph'" class="editor-actions">
        <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.createNodeButton" placement="bottom">
          <button @click="openCreateNodeModal" class="glass-button"><span>Create Node</span></button>
        </RichTooltip>
        <button v-else @click="openCreateNodeModal" class="glass-button"><span>Create Node</span></button>
        <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.configureGraph" placement="bottom">
          <button @click="openConfigureGraphModal" class="glass-button"><span>Configure Graph</span></button>
        </RichTooltip>
        <button v-else @click="openConfigureGraphModal" class="glass-button"><span>Configure Graph</span></button>
        <RichTooltip v-if="shouldShowTooltip" :content="helpContent.contextMenu.launch" placement="bottom">
          <button @click="goToLaunch" class="launch-button-primary"><span>Launch</span></button>
        </RichTooltip>
        <button v-else @click="goToLaunch" class="launch-button-primary"><span>Launch</span></button>

        <div class="menu-container" @mouseenter="showMenu = true" @mouseleave="showMenu = false">
          <div class="menu-trigger" :class="{ 'menu-trigger-active': showMenu }">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <input id="rename-workflow-name" v-model="renameWorkflowName" type="text" class="form-input" placeholder="Enter new workflow name" @keyup.enter="handleRenameSubmit" />
        </div>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <input id="copy-workflow-name" v-model="copyWorkflowName" type="text" class="form-input" placeholder="Enter new workflow name" @keyup.enter="handleCopySubmit" />
        </div>
      </div>
      <div class="modal-footer">
...
const FORM_GENERATOR_CONFIG = Object.freeze({
  graph: [{ node: 'DesignConfig', field: 'graph' }],
  node: [{ node: 'DesignConfig', field: 'graph' }, { node: 'GraphDefinition', field: 'nodes' }],
  edge: [{ node: 'DesignConfig', field: 'graph' }, { node: 'GraphDefinition', field: 'edges' }],
  memory: [{ node: 'DesignConfig', field: 'graph' }],
  vars: []
})
...
const contextMenuType = ref('pane')
const contextNodeId = ref(null)
const contextEdgeInfo = ref(null)
...
const getFlowPositionFromEvent = (event) => {
  try {
    ...
    if (!viewport || !container || !event) return getCentralPosition()
...
const onContainerContextMenu = (event) => {
  if (!vueflowContainerRef.value) return
...
const onNodeContextMenu = (params) => {
  ...
  if (!mouseEvent || !node?.id || !vueflowContainerRef.value) return
  mouseEvent.preventDefault?.()
...
const onEdgeContextMenu = (params) => {
  ...
  if (!mouseEvent || !edge || !vueflowContainerRef.value) return
  mouseEvent.preventDefault?.()
...
const deleteNodeById = async (nodeId) => {
  ...
  const nextStart = Array.isArray(sourceGraph.start) ? sourceGraph.start.filter(id => id !== nodeId) : sourceGraph.start
  const nextEnd = Array.isArray(sourceGraph.end) ? sourceGraph.end.filter(id => id !== nodeId) : sourceGraph.end
  const nextSnapshot = { ...source, graph: { ...sourceGraph, nodes: nextNodes, edges: nextEdges, start: nextStart, end: nextEnd } }
...
const deleteEdgeByEndpoints = async (fromId, toId) => {
  if (!fromId || !toId) return
  const source = yamlContent.value
  if (!source?.graph || !Array.isArray(source.graph.edges)) return
  const sourceGraph = source.graph
  let removed = false
  ...
  if (fromId === START_NODE_ID) {
    nextStart = Array.isArray(sourceGraph.start) ? sourceGraph.start.filter(id => id !== toId) : sourceGraph.start
  ...
  const nextSnapshot = { ...source, graph: { ...sourceGraph, edges: nextEdges, start: nextStart } }
  const ok = await persistYamlSnapshot(nextSnapshot)
  if (!ok) return
  await loadYamlFile()
  syncVueNodesAndEdgesData()
...
const onDeleteEdgeFromContext = async () => {
  ...
  if (!confirmed || !info?.from || !info?.to) return
...
watch(() => props.workflowName, async (newName) => { await initializeWorkflow(newName) }, { immediate: false })
...
const saveVueFlowGraph = async () => {
  try {
    ...
    const result = await postVuegraphs({ filename: key, content: JSON.stringify(flowObj) })
...
const updateNodesAndEdgesFromYaml = (preserveExistingLayout = false) => {
  try {
    const yamlNodes = Array.isArray(yamlContent.value?.graph?.nodes) ? yamlContent.value.graph.nodes : []
    const yamlEdges = Array.isArray(yamlContent.value?.graph?.edges) ? yamlContent.value.graph.edges : []
    const currentNodes = nodes.value || []
    const currentEdges = edges.value || []
    const defaultCenterPosition = getCentralPosition()
    const getDefaultCenterPosition = () => ({ x: defaultCenterPosition.x, y: defaultCenterPosition.y })
    const existingNodeById = preserveExistingLayout ? new Map(currentNodes.map(node => [node.id, node])) : null
    const existingEdgeByKey = preserveExistingLayout ? new Map(currentEdges.map(edge => [`${edge.source}-${edge.target}`, edge])) : null
    try {
      const nodeIds = (yamlNodes || []).map(n => n?.id).filter(Boolean)
      const adj = new Map()
      const indegree = new Map()
      nodeIds.forEach(id => { adj.set(id, new Set()); indegree.set(id, 0) })
      ;(yamlEdges || []).forEach(e => {
      ...
      if (q.length === 0) {
        try {
          const yamlStartList = Array.isArray(yamlContent.value?.graph?.start) ? yamlContent.value.graph.start : []
      ...
      nodeIds.forEach(id => { if (!levelById.has(id)) levelById.set(id, 0) })
      ...
      const nextNodes = yamlNodes.map((yamlNode) => {
        const id = yamlNode?.id
        if (!id) return null
        if (preserveExistingLayout && existingNodeById?.has(id)) {
          const existingNode = existingNodeById.get(id)
          return { ...existingNode, id, label: id, data: yamlNode }
        }
        const pos = positions.get(id) || getDefaultCenterPosition()
        return { id, type: 'workflow-node', label: id, position: pos, data: yamlNode }
      }).filter(Boolean)
      nodes.value = nextNodes
    } catch (err) {
      const nextNodes = yamlNodes.map((yamlNode) => ({ id: yamlNode.id, type: 'workflow-node', label: yamlNode.id, position: getDefaultCenterPosition(), data: yamlNode }))
      nodes.value = nextNodes
    }
    const nextYamlEdges = yamlEdges.map(yamlEdge => {
      ...
      const baseEdge = preserveExistingLayout && existingEdgeByKey?.has(key)
        ...
        : { id: key, source: yamlEdge.from, target: yamlEdge.to, type: 'workflow-edge' }
      return {
        ...baseEdge,
        id: key,
        ...
        target: yamlEdge.to,
        data: yamlEdge,
        markerEnd: { type: MarkerType.Arrow, width: 16, height: 16, color: (yamlEdge && yamlEdge.trigger === false) ? '#c6a0ae' : '#cf7ea2', strokeWidth: 1.5 }
      }
    })
    ...
    if (preserveExistingLayout && existingNodeById?.has(START_NODE_ID)) {
      ...
      try {
        ...
        if (yamlNodesInGraph.length) {
          ...
          startNode = { id: START_NODE_ID, type: 'start-node', label: 'Start', position: { x: minX - 100, y: avgY + 80 }, data: { id: START_NODE_ID, label: 'Start' } }
        } else {
          startNode = { id: START_NODE_ID, type: 'start-node', label: 'Start', position: getDefaultCenterPosition(), data: { id: START_NODE_ID, label: 'Start' } }
        }
      } catch (err) {
        startNode = { id: START_NODE_ID, type: 'start-node', label: 'Start', position: getDefaultCenterPosition(), data: { id: START_NODE_ID, label: 'Start' } }
      }
    ...
    const startEdges = (yamlNodes || []).map(yamlNode => {
      ...
      const baseEdge = preserveExistingLayout && existingEdgeByKey?.has(key)
        ? existingEdgeByKey.get(key)
        : { id: key, source: START_NODE_ID, target: yamlNode.id, type: 'workflow-edge' }
      return { ...baseEdge, id: key, source: START_NODE_ID, target: yamlNode.id, data: { from: START_NODE_ID, to: yamlNode.id }, markerEnd: { type: MarkerType.Arrow, width: 16, height: 16, color: '#cf7ea2', strokeWidth: 1.5 }, animated: false }
    }).filter(Boolean)
    ...
    if (!nodes.value.some(n => n.id === START_NODE_ID)) nodes.value = [startNode, ...nodes.value]
    else nodes.value = nodes.value.map(n => n.id === START_NODE_ID ? startNode : n)
  } catch (error) {
...
const generateNodesAndEdges = async (options = {}) => {
  ...
  try {
    await nextTick()
    if (options.fit) fitView?.({ padding: 0.1 })
    await saveVueFlowGraph()
  } catch (err) {
...
const syncVueNodesAndEdgesData = () => { updateNodesAndEdgesFromYaml(true) }

const updateVueFlowNodeId = (oldId, newId) => {
  if (!oldId || !newId || oldId === newId) return
  nodes.value = (nodes.value || []).map(node => {
    if (node.id !== oldId) return node
    return { ...node, id: newId, label: newId, data: node.data ? { ...node.data, id: newId } : { id: newId } }
  })
  edges.value = (edges.value || []).map(edge => {
    let source = edge.source
    let target = edge.target
    let edgeChanged = false
    if (source === oldId) { source = newId; edgeChanged = true }
    if (target === oldId) { target = newId; edgeChanged = true }
    ...
    if (edge.data) {
      ...
      if (nextFrom !== edge.data.from || nextTo !== edge.data.to) nextData = { ...edge.data, from: nextFrom, to: nextTo }
    }
    const nextEdge = { ...edge, source, target, data: nextData }
    if (edgeChanged) nextEdge.id = `${source}-${target}`
    return nextEdge
  })
  if (yamlContent.value?.graph?.start && Array.isArray(yamlContent.value.graph.start)) {
    yamlContent.value.graph.start = yamlContent.value.graph.start.map(startNodeId => startNodeId === oldId ? newId : startNodeId)
  }
  if (yamlContent.value?.graph?.end && Array.isArray(yamlContent.value.graph.end)) {
    yamlContent.value.graph.end = yamlContent.value.graph.end.map(endNodeId => endNodeId === oldId ? newId : endNodeId)
  }
}

const buildYamlWithoutNode = (nodeId) => {
  const source = yamlContent.value
  if (!source?.graph?.nodes || !Array.isArray(source.graph.nodes)) return source
  return { ...source, graph: { ...source.graph, nodes: source.graph.nodes.filter(node => node?.id !== nodeId) } }
}

const buildYamlWithoutEdge = (fromId, toId) => {
  const source = yamlContent.value
  if (!source?.graph?.edges || !Array.isArray(source.graph.edges)) return source
  let removed = false
  const filteredEdges = source.graph.edges.filter(edge => {
    if (!removed && edge?.from === fromId && edge?.to === toId) { removed = true; return false }
    return true
  })
  return { ...source, graph: { ...source.graph, edges: filteredEdges } }
}

const buildYamlWithoutVars = () => {
  const source = yamlContent.value
  if (!source || typeof source !== 'object') return source
  if (!Object.prototype.hasOwnProperty.call(source, 'vars')) return source
...
const buildYamlWithoutGraph = () => {
  const source = yamlContent.value
  if (!source || typeof source !== 'object') return source
  if (!Object.prototype.hasOwnProperty.call(source, 'graph')) return source
  const sanitized = { ...source }
...
const autoAddStartEdge = async (nextNodeId) => {
  ...
  if (workflowNodes.length === 1 && workflowNodes[0]?.id === nextNodeId) {
    ...
    if (!currentStart.includes(nextNodeId)) {
      const nextSnapshot = { ...source, graph: { ...sourceGraph, start: [...currentStart, nextNodeId] } }
      const ok = await persistYamlSnapshot(nextSnapshot)
      if (ok) {
        await loadYamlFile()
        syncVueNodesAndEdgesData()
...
const openDynamicFormGenerator = (type, options = {}) => {
  ...
  if (Object.prototype.hasOwnProperty.call(options, 'initialFormData')) formGeneratorInitialFormData.value = options.initialFormData || null
  else formGeneratorInitialFormData.value = null
...
const handleFormGeneratorSubmit = async (payload) => {
  try {
    ...
    if (previousNodeId && nextNodeId && previousNodeId !== nextNodeId) updateVueFlowNodeId(previousNodeId, nextNodeId)
    await loadYamlFile()
    syncVueNodesAndEdgesData()
    await nextTick()
    if (formGeneratorMode.value === 'create' && pendingNodePosition.value && nextNodeId) {
      ...
      if (newNode) newNode.position = { x: pendingNodePosition.value.x, y: pendingNodePosition.value.y }
      pendingNodePosition.value = null
    }
    if (formGeneratorMode.value === 'create' && nextNodeId) await autoAddStartEdge(nextNodeId)
    await saveVueFlowGraph()
  } catch (error) {
...
const handleFormGeneratorCopy = (payload) => {
  try {
    ...
    if (copied && typeof copied === 'object') copied.id = ''
    setTimeout(() => {
      openDynamicFormGenerator('node', { mode: 'create', initialFormData: copied })
    }, 0)
  } catch (error) {
...
const openNodeEditor = (nodeId) => {
  ...
  openDynamicFormGenerator('node', { initialYaml: sanitizedYaml, initialFormData: yamlNodeContent, mode: 'edit' })
}
...
const openEdgeEditor = (fromId, toId, fallbackData = null) => {
  ...
  openDynamicFormGenerator('edge', { initialYaml: sanitizedYaml, initialFormData: edgeData, mode: 'edit' })
}
...
const onEdgeClick = (event) => {
  ...
  if (!clickedEdge?.id) return
  const fromId = clickedEdge.data?.from || clickedEdge.source || ''
  const toId = clickedEdge.data?.to || clickedEdge.target || ''
  if (fromId === START_NODE_ID || toId === START_NODE_ID) return
  if (!fromId || !toId) return
  const fallbackData = { from: fromId, to: toId }
  if (clickedEdge.data?.condition !== undefined) fallbackData.condition = clickedEdge.data.condition
  if (clickedEdge.data?.trigger !== undefined) fallbackData.trigger = clickedEdge.data.trigger
  openEdgeEditor(fromId, toId, fallbackData)
}

const onNodeDragStop = () => { saveVueFlowGraph() }
...
const onConnect = async (connection) => {
  ...
  if (connection.source === START_NODE_ID) {
    const source = yamlContent.value
    if (!source?.graph) {
      setTimeout(() => { isCreatingConnection.value = false }, 10)
      return
    }
    const sourceGraph = source.graph
    const currentStart = Array.isArray(sourceGraph.start) ? sourceGraph.start : []
    if (!currentStart.includes(connection.target)) {
      const nextSnapshot = { ...source, graph: { ...sourceGraph, start: [...currentStart, connection.target] } }
      const ok = await persistYamlSnapshot(nextSnapshot)
      if (ok) {
        await loadYamlFile()
        syncVueNodesAndEdgesData()
  ...
  const edgeAlreadyExistsInYaml = yamlEdges.some(e => e.from === connection.source && e.to === connection.target)
  const edgeAlreadyExistsInGraph = edges.value.some(e => e.source === connection.source && e.target === connection.target)
  if (edgeAlreadyExistsInYaml || edgeAlreadyExistsInGraph) {
    setTimeout(() => { isCreatingConnection.value = false }, 10)
    return
  }
  const autoCreatedEdgeIndex = edges.value.findIndex(edge => edge.source === connection.source && edge.target === connection.target)
  if (autoCreatedEdgeIndex !== -1) edges.value.splice(autoCreatedEdgeIndex, 1)
  openDynamicFormGenerator('edge', { initialFormData: { from: connection.source, to: connection.target, condition: { type: 'function', config: { name: 'true' } }, trigger: true }, mode: 'create' })
  setTimeout(() => { isCreatingConnection.value = false }, 100)
}

const openCreateEdgeModal = () => { openDynamicFormGenerator('edge', { mode: 'create' }) }

const goToLaunch = () => {
  if (!workflowName.value) return
  const fileName = workflowName.value.endsWith('.yaml') ? workflowName.value : `${workflowName.value}.yaml`
  const resolved = router.resolve({ path: '/launch', query: { workflow: fileName } })
...
const handleRenameSubmit = async () => {
  if (!renameWorkflowName.value.trim()) return
  ...
  if (result.success) {
    ...
    try {
      ...
      if (oldVueGraphResult.success && oldVueGraphResult.content) {
        await postVuegraphs({ filename: newWorkflowKey, content: oldVueGraphResult.content })
      }
    } catch (error) {}
...
const handleCopySubmit = async () => {
  if (!copyWorkflowName.value.trim()) return
  ...
  if (result.success) {
    ...
    try {
      ...
      if (sourceVueGraphResult.success && sourceVueGraphResult.content) {
        await postVuegraphs({ filename: targetWorkflowKey, content: sourceVueGraphResult.content })
      }
    } catch (error) {}
    alert(result.message)
    closeCopyModal()
...
.tab-buttons { display: flex; gap: 4px; height: 100%; }
...
.editor-actions { display: flex; gap: 12px; align-items: center; height: 100%; }
...
.glass-button:hover {
  ...
  box-shadow: 0 12px 24px rgba(212, 142, 175, 0.16);
}
...
.menu-container { position: relative; z-index: 3; height: 100%; }
.menu-trigger {
  width: 40px;
  height: 100%;
  display: flex;
...
.menu-trigger:hover { background: rgba(255, 241, 246, 0.7); color: #c0618c; }
...
.menu-item:hover { background-color: rgba(255, 241, 246, 0.8); color: #b05c81; }
...
.yaml-error {
  padding: 12px 20px;
  background-color: rgba(255, 221, 231, 0.82);
  border-bottom: 1px solid rgba(220, 121, 159, 0.24);
  color: #b2456d;
  font-size: 14px;
}
...
.yaml-textarea::-webkit-scrollbar { display: none; }
.yaml-error-border { border: 2px solid #d45d8b !important; }
...
.context-menu {
  ...
  border-radius: 14px;
  padding: 6px;
  box-shadow: 0 16px 30px rgba(212, 142, 175, 0.16);
  border: 1px solid rgba(223, 156, 185, 0.18);
  backdrop-filter: blur(12px);
  z-index: 5;
}
...
.context-menu-item:hover { background-color: rgba(255, 241, 246, 0.8); color: #b05c81; }
...
.modal-overlay {
  ...
  right: 0;
  bottom: 0;
  background-color: rgba(255, 240, 246, 0.38);
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 1000;
...
.close-button:hover { color: #c0608b; }
...
.form-input:focus {
  border-color: rgba(223, 156, 185, 0.36);
  background: rgba(255, 250, 252, 0.96);
}

.form-input::placeholder { color: #b28a99; }
...
.submit-button:hover:not(:disabled) {
  transform: translateY(-1px);
  box-shadow: 0 10px 20px rgba(226, 150, 182, 0.18);
}
